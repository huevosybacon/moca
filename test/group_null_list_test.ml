open Group_null_list
;;

open Gentest
;;

testing ("Group_null_list (automatic) - seed: 707000267")
;;
...
testi
  ...
  (let x0 = opp zero in
   ...
   let x3 = add [ (opp zero); (opp zero); (add [ zero; zero; zero ]) ] in
...
testi
  ...
  (let x0 = opp (opp zero) in
   ...
   let x2 = opp zero in
   let x3 = opp zero in
   let x4 = add [ (opp zero); (opp zero); (add [ zero; zero; zero ]) ] in
   add [ x0; (add [ x1; x2 ]); (add [ x3; x4 ]) ] =
   add [ x0; x1; x2; x3; x4 ])
;;
...
testi
  ...
  (let x0 =
   ...
   let x2 = opp (opp zero) in
   let x3 = add [ (opp zero); (opp zero); (add [ zero; zero; zero ]) ] in
   let x4 = add [ (add [ zero; zero; zero ]); (opp zero); (opp zero) ] in
   add [ x0; (add [ x1; x2 ]); (add [ x3; x4 ]) ] =
   add [ x0; x1; x2; x3; x4 ])
;;
...
testi
  ...
  (let x0 = opp (opp zero) in
   ...
   let x2 = opp (opp zero) in
   let x3 = add [ (opp zero); (opp zero); zero ] in
   add [ x0; x1; x2; x3 ] = add [ x1; x0; x3; x2 ])
;;
...
testi
  (37)
  (let x0 = add [ (add [ zero; zero; zero ]); (opp zero); (opp zero) ] in
   let x1 = add [ (opp zero); (add [ zero; zero; zero ]); (opp zero) ] in
   add [ x0; zero; x1 ] = add [ x0; x1 ])
;;
...
testi
  ...
  (let x0 =
     add
       [ (add [ zero; zero; zero ]); (add [ zero; zero; zero ]); (opp zero) ] in
   let x1 = opp (opp zero) in add [ x0; zero; x1 ] = add [ x0; x1 ])
;;
...
testi (41) (let x0 = opp (opp zero) in add [ zero; x0 ] = x0)
;;
...
testi (46) (let x0 = opp (opp zero) in add [ x0; zero; zero ] = x0)
;;
...
testi (48) (let x0 = opp (opp zero) in add [ x0; zero; zero ] = x0)
;;
